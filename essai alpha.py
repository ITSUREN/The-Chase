import turtle,random,os
from playsound import playsound
from time import sleep

#path to resources maker
dirname = os.path.dirname(os.path.abspath("__file__"))
redirect = os.path.join(dirname,'Resources\\')
os.chdir(redirect)
check="False"

def nothing():
    print("enter hit")
    turtle.bye()

#windows
def pineapplepen():
    global pen,ch,run,block,blocka,blockb,s,speed_ch,speed_run,fps
    wn=turtle.Screen()
    fps=1920
    wn.setup(width = 1.0, height = 1.0)                         #for full screem
    canvas = wn.getcanvas()                                     #gets canvas
    root = canvas.winfo_toplevel()                              #gets title bar to root var
    root.overrideredirect(1)                                    #removes the minimize,close and other title bar components
    wn.title("The Chase: 2Players")
    wn.addshape("runner2.gif")
    wn.addshape("BOMB.gif")
    wn.addshape("ch_left.gif")
    wn.addshape("ch_right.gif")
    wn.addshape("runner2_right.gif")
    wn.addshape("plasma_left.gif")
    wn.addshape("missile.gif")
    wn.addshape("Octopus.gif")
    wn.addshape("bullet.gif")
    wn.addshape("beam_up.gif")
    wn.addshape("beam_down.gif")
    wn.addshape("ufo_left.gif")
    wn.addshape("ufo_right.gif")
    wn.addshape("bg.gif")
    wn.bgpic("bg.gif")
    wn.tracer(0)
    s=30
    speed_ch=posx/1000
    speed_run=posx/1000

#pen
    pen=turtle.Turtle()
    pen.speed(0)
    pen.color("white")
    pen.penup()
    pen.hideturtle()
    pen.goto(-100, 0)

#chaser
    ch=turtle.Turtle()
    ch.speed(0)
    ch.shape("Octopus.gif")
    ch.penup()
    ch.shapesize(stretch_wid=2, stretch_len=2)
    ch.goto(-posx, 0)
    global q,ch_x,ch_y
    q=30
    ch_x=0.5
    ch_y=0


#runner
    run=turtle.Turtle()
    run.speed(0)
    run.shape("ufo_left.gif")
    run.penup()
    run.goto(posx, 0)
    global run_x,run_y
    run_x= -0.7
    run_y=0

#blocks
    block=turtle.Turtle()
    block.shape("beam_down.gif")
    block.speed(0)
    block.penup()
    block.goto(random.randint(-posx, posx), 290)
    p=0.3

    blocka=turtle.Turtle()
    blocka.shape("beam_up.gif")
    blocka.speed(0)
    blocka.penup()
    blocka.goto(random.randint(-posx, posx), -290)
    e=0.3

    blockb=turtle.Turtle()
    blockb.shape("beam_down.gif")
    blockb.speed(0)
    blockb.penup()
    blockb.goto(random.randint(-posx, posx), 290)
    r=0.3

    gun=turtle.Turtle()
    global shoot,ammo,gun_x,gun_y
    gun.shape("bullet.gif")
    gun.speed(0)
    gun.penup()
    gun.goto(1600, 1600)
    shoot = False
    ammo =5
    gun_x=0
    gun_y=0
    w=20

    def reset():
        pen.clear()
        pen.hideturtle()
        ch.hideturtle()
        run.hideturtle()
        block.hideturtle()
        blocka.hideturtle()
        blockb.hideturtle()
        global s, ammo, game, gun_y, gun_x
        game = False
        gun_x=0
        gun_y=0
        ch.goto(-posx, 0)
        run.goto(posx, 0)
        gun.goto(1600, 1600)
        block.goto(random.randint(-posx, posx), 290)
        blocka.goto(random.randint(-posx, posx), -290)
        blockb.goto(random.randint(-posx, posx), 290)
        ammo = 5
        s=30
        pineapplepen()

#control chaser
    def ch_up():
        global ch_x,ch_y
        ch_x=0
        ch_y= speed_ch
    def ch_down():
        global ch_x,ch_y
        ch_x=0
        ch_y= -speed_ch
    def ch_right():
        global ch_x,ch_y
        ch_x=speed_ch
        ch_y= 0
    def ch_left():
        global ch_x,ch_y
        ch_x=-speed_ch
        ch_y= 0

#control runner
    def run_up():
        global run_x,run_y
        run_x=0
        run_y= speed_run
    def run_down():
        global run_x,run_y
        run_x=0
        run_y= -speed_run
    def run_right():
        run.shape("ufo_right.gif")
        global run_x,run_y
        run_x=speed_run
        run_y= 0
    def run_left():
        run.shape("ufo_left.gif")
        global run_x,run_y
        run_x=-speed_run 
        run_y= 0

    def shoot_gun():
        global gun_x, gun_y, shoot, ammo
        if not shoot and ammo > 0:
            gun.goto(ch.xcor(), ch.ycor())
            gun_x= ch_x*2
            gun_y= ch_y*2
            shoot = True
            ammo -= 1
    
    def reloader():
        global ammo
        ammo = 5

    def exiter():
        turtle.bye()
        

#keyboard binding
    wn.listen()

    wn.onkeypress(ch_up, "w")
    wn.onkeypress(ch_down, "s")
    wn.onkeypress(ch_right, "d")
    wn.onkeypress(ch_left, "a")

    wn.onkeypress(run_up, "Up")
    wn.onkeypress(run_down, "Down")
    wn.onkeypress(run_right, "Right")
    wn.onkeypress(run_left, "Left")

    wn.onkeypress(nothing,"Return")
    wn.onkeypress(shoot_gun, "space")
    wn.onkeypress(reloader, "r")
    wn.onkeypress(reset, "z")
    wn.onkeypress(exiter,"x")

#checks:
    while True:
        wn.update()     #update sthe screen

        if block.ycor() < -290:
            block.goto(random.randint(-posx, posx), 290)
            pen.setx(-100)
            pen.clear()
            pen.write(s, font=("Arial", 25))  
            s -= 1
            p = random.randint(50, 100)
            p = p / 10

        if blocka.ycor()>290:
            blocka.goto(random.randint(-posx,posx), -290)
            e = random.randint(50, 100)
            e = e / 10

        if blockb.ycor()< -290:
            blockb.goto(random.randint(-posx, posx), 290)
            r = random.randint(50, 100)
            r = r / 10

        if (run.xcor()> ch.xcor()-q and run.xcor() < ch.xcor()+q) and (run.ycor()> ch.ycor()-q and run.ycor() < ch.ycor()+q):
            pen.clear()
            playsound("game_over.mp3")
            pen.setx(-300)
            pen.write("Runner is Caught!! The Chaser Wins.\n press 'z' to play again \n and 'x' to close", font=("Arial", 25))  
            t = input("You exited the game")
            print(t)
            turtle.bye()

        if ((block.xcor() > ch.xcor() - q and block.xcor() < ch.xcor() + q) and (block.ycor() > ch.ycor() - q and block.ycor() < ch.ycor() + q)) or ((blockb.xcor() > ch.xcor() - q and blockb.xcor() < ch.xcor() + q) and (blockb.ycor() > ch.ycor() - q and blockb.ycor() < ch.ycor() + q)) or ((blocka.xcor() > ch.xcor() - q and blocka.xcor() < ch.xcor() + q) and (blocka.ycor() > ch.ycor() - q and blocka.ycor() < ch.ycor() + q)):
            wn.bgpic("explosion_megumin.gif")
            wn.update()
            playsound("megumin_chant.mp3")
            pen.clear()
            playsound("explosion.mp3")
            playsound("game_over.mp3")
            pen.setx(-300)
            pen.write("Chaser got hit. Runner wins!!\n press 'z' to play again \n and 'x' to close", font=("Arial", 25))  
            t = input("You exited the game")
            print(t)
            turtle.bye()

        if (run.xcor() > gun.xcor() - w and run.xcor() < gun.xcor() + w) and (gun.ycor() - w < run.ycor() < gun.ycor() + w):
            pen.clear()
            playsound("explosion.mp3")
            pen.setx(-300)
            pen.write("The Runner got hit. The Chaser Wins.\n press 'z' to play again \n and 'x' to close", font=("Arial", 25))
            t = input("You exited the game")
            print(t)
            turtle.bye()


        if s==0:
            pen.clear()
            playsound("game_over.mp3")
            pen.setx(-300)
            pen.write("TIME OVER./n press 'z' to play again", font=("Arial", 25))  
            t = input("You exited the game")
            print(t)
            

        block.sety(block.ycor() - p)
        blocka.sety(blocka.ycor()+ e)
        blockb.sety(blockb.ycor() - r)

        ch.setx(ch.xcor()+ch_x)
        ch.sety(ch.ycor()+ch_y)
        run.setx(run.xcor()+run_x)
        run.sety(run.ycor()+run_y)
        gun.setx(gun.xcor()+gun_x)
        gun.sety(gun.ycor()+gun_y)
#bouncer
        if ch.xcor()>posx or ch.xcor()< -posx:
            ch_x = -ch_x
        if ch.ycor()>posy or ch.ycor()< -posy:
            ch_y = -ch_y

        if run.xcor()>posx or run.xcor()< -posx:
            run_x = -run_x
        if run.ycor()>posy or run.ycor()< -posy:
            run_y = -run_y
        if (gun.ycor()>(posy+40 )or gun.ycor()< -(posy+40 )) or (gun.xcor()>(posy+40 ) or gun.xcor()<-(posy+40 )):
            shoot = False
        sleep(1/fps)
#it joins loading module with the main module like a bridge

#settings
def settings():
    pen.setx(-300)
    pen.clear()
    pen.write("this is yet to be implemented.\nGood job finding this one.\nHear this music instead", font=("Arial", 25))  
    playsound("backgroundsound.mp3")
    pen.clear()
    turtle.bye()
#loading screen
def ihaveapenihaveanapple():
    global wn,pen,posx,posy,heck
    check="True"
    wn=turtle.Screen()
    wn.setup(width = 1.0, height = 1.0)
    wn.title("The Chase: 2Players")
    wn.bgpic("begining_bg.png")
    wn.addshape("settings.gif")
    posx = wn.window_width()//2-50
    posy = wn.window_height()//2-30
    

    sett=turtle.Turtle()
    sett.shape("settings.gif")
    sett.penup()
    sett.goto(posx, posy)

    pen = turtle.Turtle()
    pen.hideturtle()
    def uhhhh():
        pen.clear()
        pineapplepen()

    def color_coder(text,i,j,size):
        pen.penup()
        pen.setx(i)
        pen.sety(j)
        global color
        color=["violet","indigo","blue","green","yellow","orange","red","cyan"] #list of colors respectively
        global ren
        for ren in range(len(color)):    #accessess the list of colors
            wn.listen()
            wn.onkey(uhhhh,"Return")
            wn.onkey(settings,"u")
            pen.color(color[ren])
            pen.write(text, font=("Arial", size))
            sleep(0.25)

    pen.clear()
    i= -400
    j=0
    text="THE CHASE: 2PLAYERS"  #this will be displayed
    size=50
    color_coder(text,i,j,size)
    j=-100
    i=-150
    size=25
    text="press enter to begin"  #this will be displayed
    color_coder(text,i,j,size)
    j=-200
    i=-150
    size=10
    text="you cannot leave when the game is proceeding.\nThis setting was implemented by itsuren but you can exit by entering enter.\nThe Game admin is Ninamhang."  #this will be displayed (\n means hitting enter or new line)
    color_coder(text,i,j,size)
    sleep(3)
    pen.clear()
    uhhhh()

ihaveapenihaveanapple() #this is required to initiate the program else the program wont run



